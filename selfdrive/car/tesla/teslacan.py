...
class TeslaCAN:
  ...
  def hybrid_longitudinal(self, acc_state, accel, das_control, cntr, speed, gas_pressed):
    ...
    if speed <= 25 or gas_pressed:
      ...
      factor = min(time.time() - self.max_op_time, max_fade) / max_fade
      ...
      factor = min(time.time() - self.max_tacc_time, max_fade) / max_fade
    ...
    if (-0.5 > accel > das_control["DAS_accelMin"]) or gas_pressed:
      ...
      min_fade = 1.0  # 1 second
      factor = min(time.time() - self.min_tacc_time, min_fade) / min_fade
